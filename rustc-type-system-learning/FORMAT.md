...
|---|---|---:|---|---|
| E02.3 | 未通过 | 0 / 1 | 精确写出目标概念或源码分支 | E05.2 |
| E01.2 | 部分通过 | 0.5 / 1 | 精确写出尚需确认的判断 | E05.1 |
...
- 每次完成正式评分后，`待复核小问（未满分）` 必须紧跟 `总评`，列出当前得分低于该小问满分的全部小问；即使本章已达 `mastered`，仍按当前有效成绩列出。未评分时写“待评分”，全部满分时写“无”。
- 清单使用精确题号（如 `E02.3`），`0 分 → 未通过`，`大于 0 但未满分 → 部分通过`；先列“未通过”，再列“部分通过”，各组按题号排序。状态必须用文字表示，不只依赖颜色或符号。
- `需要补齐` 写一个可检查的知识点或结论，`复核位置` 指向现有讲评、补充题或待安排动作。复答后按当前事实视图更新该清单：达到满分的小问移出，原始回答仍留在 `exercises.md`。
- 从 1.3.0 起新建或再次更新的评分文件应用此清单；较早的已归档章节保留原评分，后续复核该章时再补齐清单。
...
| 2026-09-25 | 1.3.0 | 评分模板增加置顶“待复核小问（未满分）”清单，明确标记未通过、部分通过及对应复核位置。 |
