...
impl<'ra, 'tcx> Resolver<'ra, 'tcx> {
    ...
    fn resolve_import<'r>(mut self: CmResolver<'r, 'ra, 'tcx>, import: Import<'ra>) -> usize {
        ...
        let (source, target, bindings) = match import.kind {
            ImportKind::Single { source, target, ref decls, .. } => {
                (source, target, decls)
            }
            ImportKind::Glob { .. } => {
        ...
        if source.is_path_segment_keyword() {
            // TODO(mu00): Check importing ExternPrelude/crate/$crate/super at the end here
        }
    ...
    fn finalize_import(&mut self, import: Import<'ra>) -> Option<UnresolvedImportError> {
        ...
        let (ident, target, bindings, import_id) = match import.kind {
            ImportKind::Single { source, target, ref decls, id, .. } => {
                (source, target, decls, id)
            }
