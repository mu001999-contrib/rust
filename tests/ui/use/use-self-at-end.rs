...
pub mod x {
    ...
    pub mod y {
        ...
        pub use z::self::self as z1; //~ ERROR `self` in paths can only be used in start position or last position
        pub use z::{self::{self}}; //~ ERROR `self` in paths can only be used in start position or last position
        pub use z::{self::{self as z2}}; //~ ERROR `self` in paths can only be used in start position or last position
        ...
        type J = super::Trait::self; //~ WARN trait objects without an explicit `dyn` are deprecated
